...
- (void)creatSubViewsAction {
    ...
    _listTableView.rowHeight = 300;
    _listTableView.estimatedRowHeight = 0;
    _listTableView.estimatedSectionFooterHeight = 0;
    _listTableView.estimatedSectionHeaderHeight = 0;
    _listTableView.delegate = self;
    _listTableView.dataSource = self;
    [_listTableView registerClass:[HomeListCell class] forCellReuseIdentifier:@"HomeListCell"];
...
- (void)loadListAction:(BOOL)isfooter {
    ...
    NSString *filePath = [[NSBundle mainBundle] pathForResource:@"人员列表" ofType:@"json"];
    ...
    if (_purseType == 0) {
        ...
        items = [items sortedArrayUsingComparator:^NSComparisonResult(id  _Nonnull obj1, id  _Nonnull obj2) {
            return 1;
        }];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row < _dataArray.count) {
        ...
        PersonDetialController *ctrl = [[PersonDetialController alloc] init];
        ctrl.model = model;
