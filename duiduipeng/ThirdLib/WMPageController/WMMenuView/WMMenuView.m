...
- (void)addScrollView {
    ...
    if (_bottomLineColor) {
        ...
        self.bottomLineView.backgroundColor = cWhiteColor; // CYC666 找了好久，我就是不想要这个颜色
    }
    self.bottomLineView.frame = CGRectMake(0, height-1, cScreenWidth, 1);
    [self.scrollView insertSubview:self.bottomLineView atIndex:1];
}
...
- (void)calculateItemFrames {
    ...
    self.bottomLineView.c_width = contentWidth;
}
...
- (void)addProgressViewWithFrame:(CGRect)frame isTriangle:(BOOL)isTriangle hasBorder:(BOOL)hasBorder hollow:(BOOL)isHollow cornerRadius:(CGFloat)cornerRadius {
    ...
    [self.scrollView insertSubview:self.progressView aboveSubview:self.bottomLineView];
